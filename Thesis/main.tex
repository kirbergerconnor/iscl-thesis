...
Figure \ref{fig:pcaanalysis} shows the results of the dimensionality reduction performed on the data using PCA.\footnote{A script was used to increase the readability of the annotations in the graph: https://github.com/Phlya/adjustText} The top two principal components are TTR and Character Distribution Entropy, which contribute to 62\% and 17\% of the variance, respectively. Combined, this only accounts for 79\% of the total variance.
...
As Table \ref{tbl:supervisedevaluation} shows, both classifiers achieved exactly the same precision, recall, and F\textsubscript{1}-scores.
...
\begin{table}[!h]
  ...
  \hline
    \textbf{Model} & \textbf{F\textsubscript{1}-score} & \textbf{Precision} & \textbf{Recall} \\ \hline
      ...
      One-Class SVM & 0.73 & 0.80 & 0.67 \\ \hline
...
\begin{figure}[!h]
  ...
  \end{minipage}
    \caption{SHAP Analysis of Fine-tuned Supervised Models: Decision Tree (left) Random Forest (right).}
...
\begin{figure}[!h]
  ...
  \end{minipage}
    \caption{SHAP Analysis of Fine-tuned Unsupervised Models: One-Class SVM (left) and Isolation Forest (right).}
...
\begin{figure}[!h]
  ...
  \end{minipage}
    \caption{Granular SHAP Analysis of Fine-tuned Unsupervised Models: One-Class SVM (left) and Isolation Forest (right).}
