...
Baseline models, one for each corpus, were initiated using default parameters. The word lists were then loaded into the models, again with default parameters except for \texttt{count\_modifier}, which was set to a specified logarithmic equation, expressed as: \[\lfloor\log_2(x + 1)\rceil\]
...
To identify the specific linguistic features which convey the most meaningful information in the data, PCA was performed. PCA is an unsupervised method for linear dimensionality reduction that transforms data from a high-dimensional space to a lower one while preserving as much variance or essential information in the data as possible \parencite{Deng2024article}. 

Two principal components were used. Technically, maximizing the number of informative features---according to calculations of explained variance---to preserve in this data would actually lead to using more, likely three (depending on the threshold). However, reduction to only two dimensions instead allows for easier visual interpretation, especially in the context of identifying possible outliers.
...
Additionally, to prevent bias towards the majority class, the balanced class weights were used for both models; this adjusts the misclassification cost for each class with weights that are inversely proportionate to their frequency in the data \parencite{Han2021article}. 
...
In addition to the same aforementioned Decision Tree parameters is \texttt{n\_estimators}, which is the number of trees, and \texttt{bootstrap}, which refers to the use of bootstrap sampling.
...
The engineered feature set was also used for unsupervised anomaly detection (also called outlier detection), a task for identifying outliers (and inliers, by extension) in a given dataset. This means the models train and predict on the same data to determine possible outliers, and it differs slightly from the task of novelty detection, which detects outliers in new, previously unseen data. 

For these methods, fine-tuning was again performed using a grid search approach. A notable difference with these models from the previously discussed supervised classifiers in Section \ref{ssec:classification}, however, is the lack of use of weighted classes to balance the dataset, since anomaly detection inherently assumes an imbalanced distribution of classes. 
...
Much like the classification with Random Forest, feature scaling is not required for Isolation Forest, and doing so has no effect. Table \ref{tbl:isolationforestparams} shows the hyperparameters that were fine-tuned along with the optimal values found for each. All others were left as their default values.
...
The \texttt{n\_neighbors} parameter refers to the number of nearest neighbors being considered, which were determined here using a brute-force search \texttt{algorithm}, and \texttt{metric} is the formula for calculating distances between datapoints, for which the Euclidean distance was found to be best.

\subsection{Model Evaluation}
\label{ssec:modelevaluation}

Precision, recall, and F1-score were calculated for all five models, given by Equations \ref{eq:precision}, \ref{eq:recall}, and \ref{eq:f1score}, respectively. These are standard metrics which are commonly used in machine learning tasks, including for supervised classification and unsupervised anomaly detection. 
...
It is important to note, however, that because of the limited size of the dataset, evaluation of both the supervised binary classifiers and the unsupervised anomaly detection models proved challenging, as there was not enough data to create holdout sets for testing. As a result, the scores measuring their performance do not sufficiently indicate their ability to make predictions on new, unseen data. 

To evaluate the supervised classifiers, the numbers of True Positive (TP), True Negative (NP), False Positive (FP), and False Negative (FN) predictions from all iterations of the leave-one-out validation process during fine-tuning were aggregated together. For a given estimator (combination of parameters), these were then used to calculate precision, recall, and F1-score metrics. Similarly, for the unsupervised anomaly detection models, each metric was calculated for each estimator during fine-tuning. In both tasks, the estimator which achieved the highest F1-score was determined to have the optimal parameters for the respective model.

\subsection{Feature Importance}
\label{ssec:featureimportances}

In order to further analyze the selected linguistic features, their importances were also calculated. Feature importance is a measurement of the extent that each feature contributes to model prediction, consequently indicating how relevant or useful a particular feature is (or is not). In the context of this investigation into constructed versus natural language, calculating these can provide insight into the discriminative ability that some language features may provide over others.

To measure this, the SHapley Additive exPlanation (SHAP) method is used. 

\newpage
\section{Results}
\label{sec:results}

This section starts by reporting the results of the methods implemented for, and the measurements derived from, the feature engineering. Each feature and their language distributions are further analyzed using one-dimensional graphs as well. Following this are the results of dimensionality reduction on the data, then performances of each of the models used for supervised classification and unsupervised anomaly detection. Lastly, the findings for the calculations of feature importance for each of the models is reported.

\subsection{Results of Feature Engineering}
\label{ssec:featureengineeringresults}

Table \ref{tbl:featurestable} shows the full set of features for each language of the dataset and their respective measurements rounded to three decimals places. Additionally, the column \textit{Type} shows their labels as either constructed or natural languages (\textit{con} or \textit{nat}, respectively).
...
\begin{table}[!h]
  ...
  \begin{tabular}{|l|l|l|l|l|l|l|l|l|l|l|l|l|l|l|}
    \hline
        \textbf{Language} & \rotatebox{90}{\textbf{Type}} & \rotatebox{90}{\textbf{Average Word Length}} & \rotatebox{90}{\textbf{Average Sentence Length}} & \rotatebox{90}{\textbf{TTR}} & \rotatebox{90}{\textbf{MATTR}} & \rotatebox{90}{\textbf{MTLD}} & \rotatebox{90}{\textbf{Morpheme TTR}} & \rotatebox{90}{\textbf{Average Segmentations Per Word}} & \rotatebox{90}{\textbf{Average Forms Per Stem}} & \rotatebox{90}{\textbf{Character Distribution Entropy}} & \rotatebox{90}{\textbf{Word Distribution Entropy}} & \rotatebox{90}{\textbf{Text Entropy}} & \rotatebox{90}{\textbf{Lexical Entropy}} & \rotatebox{90}{\textbf{Reverse Lexical Entropy}} \\ \hline
    ...
    \end{tabular}
  \end{adjustbox}
  \caption{Complete set of all 13 features for every language in the data, as well as each language's label (\textit{Type}), which represent either 'constructed' or 'natural'. Each feature's value is rounded to 3 decimal places.}
  \label{tbl:featurestable}
\end{table}

To visualize these features individually, 
...
\subsection{Results of Dimensionality Reduction}
\label{ssec:dimensionalityreductionresults}

Figure \ref{fig:pcaanalysis} shows the results of the dimensionality reduction performed on the data using PCA\footnote{A script was used to increase readability of the text in the graph: https://github.com/Phlya/adjustText}. The top two principal components are TTR and Character Distribution Entropy, which contribute to 62\% and 17\% of the variance, respectively. Combined, this only accounts for 79\% of the total variance.

\begin{figure}[!h]
  \centering
        \includegraphics[width=1.0\textwidth]{PCA_of_Features.png}
        \caption{Principal Component Analysis on the feature set.}
...
\begin{table}[!h]
  ...
  \hline
      ~ & \rotatebox{90}{\textbf{Average Word Length}} & \rotatebox{90}{\textbf{Average Sentence Length}} & \rotatebox{90}{\textbf{TTR}} & \rotatebox{90}{\textbf{MATTR}} & \rotatebox{90}{\textbf{MTLD}} & \rotatebox{90}{\textbf{Morpheme TTR}} & \rotatebox{90}{\textbf{Average Segmentations Per Word}} & \rotatebox{90}{\textbf{Average Forms Per Stem}} & \rotatebox{90}{\textbf{Character Distribution Entropy}} & \rotatebox{90}{\textbf{Word Distribution Entropy}} & \rotatebox{90}{\textbf{Text Entropy}} & \rotatebox{90}{\textbf{Lexical Entropy}} & \rotatebox{90}{\textbf{Reverse Lexical Entropy}} \\ \hline
      \textbf{PC1} & 0.312 & -0.152 & 0.343 & 0.271 & 0.284 & -0.25 & 0.318 & 0.328 & 0.042 & 0.323 & 0.19 & -0.305 & -0.319 \\ \hline
      \textbf{PC2} & -0.048 & 0.243 & 0.062 & 0.364 & 0.239 & 0.365 & -0.154 & 0.017 & 0.561 & 0.214 & 0.318 & 0.283 & 0.214 \\ \hline
  \end{tabular}
  \end{adjustbox}
  \caption{Loadings for Each Principal Component}
...
Figure \ref{fig:decisiontree} shows the resulting Decision Tree. To briefly explain how this is interpreted, for each node of the tree except the terminal ones, the top line is the condition for the subsequent split, and each split partitions the remaining number of samples for which said condition is either true or false. For example, the condition for the root node is \texttt{MTLD <= 70.824}, which is true for 9 samples and false for the other 15. In addition, \texttt{value} shows the weighted counts for both classes, with \texttt{class} being equal to the majority class at that split, and \texttt{gini} ranges from 0.0 (the samples all belong to one class) to 0.5 (the samples are evenly distributed, referred to as maximum impurity). 
...
Therefore, this indicates the features MTLD, Average Sentence Length, and Average Word Length were the best for classification.

\begin{figure}[!h]
  \centering
    \includegraphics[width=0.5\textwidth]{CM_decision_tree.png}
    \caption{Confusion Matrix for Decision Tree Classifier}
    \label{fig:cmdecisiontree}
\end{figure}

Based on the confusion matrix seen in Figure \ref{fig:cmdecisiontree}, the Decision Tree 

\begin{figure}[!h]
  \centering
    \includegraphics[width=0.5\textwidth]{CM_random_forest.png}
...
% \begin{figure}[!h]
%   \centering
%     \includegraphics[width=0.5\textwidth]{RF_gini_importances.png}
%     \caption{Gini Importances for Random Forest}
%     \label{fig:rfginiimportances}
% \end{figure}

% \begin{figure}[!h]
%   \centering
%     \includegraphics[width=0.5\textwidth]{RF_permutation_importances.png}
%     \caption{Permutation Importances for Random Forest}
%     \label{fig:rfpermutationimportances}
% \end{figure}
...
\begin{table}[!h]
  ...
  \hline
      ...
      Decision Tree & 0.83 & 0.83 & 0.83 \\ \hline
      Random Forest & 0.91 & 1.0 & 0.83 \\ \hline
  \end{tabular}
  \caption{Precision, Recall, and F1-scores of Supervised Binary Classification Models}
...
\begin{figure}[!h]
  \centering
    \includegraphics[width=0.5\textwidth]{CM_ocsvm.png}
    \caption{Confusion Matrix for One-Class SVM}
    \label{fig:cmocsvm}
\end{figure}
...
\begin{figure}[!h]
  \centering
    \includegraphics[width=0.5\textwidth]{IF_SHAP_values.png}
    \caption{SHAP Values of Isolation Forest}
    \label{fig:isolationforestshapvalues}
\end{figure}
...
\begin{table}[!h]
  ...
  \hline
    \textbf{Model} & \textbf{F\textsubscript{1}-score} & \textbf{Precision} & \textbf{Recall} \\ \hline
      Isolation Forest & 0.67 & 0.67 & 0.67 \\ \hline
      One-Class SVM & 0.8 & 1.0 & 0.67 \\ \hline
      Local Outlier Factor & 0.67 & 0.67 & 0.67 \\ \hline
  \end{tabular}
  \caption{Precision, Recall, and F1-scores of Unsupervised Anomaly Detection Models}
  \label{tbl:unsupervisedevaluation}
\end{table}

\subsection{Results of Calculating Feature Importance}
...
\begin{table}[!h]
  ...
  count\_modifier & $\lfloor\log_2(x + 1)\rceil$ \\
  \hline
  \end{tabular}
