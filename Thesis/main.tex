...
Regarding the findings of the SHAP analysis, measures of lexical diversity were the most influential features for all four (decision tree, random forest, one-class SVM, isolation forest) models' predictions. 

For a more granular analysis comparing feature importance for the isolation forest when predicting natural and constructed languages, SHAP force plots are shown in Figure \ref{fig:shapforceplots}. These correspond to specific instances of model predictions, providing an additive view of each feature and how it influences the final prediction. Two natural (Finnish and English) and two constructed languages (Esperanto and Volapük) are given to demonstrate a correct and incorrect prediction for each class.
