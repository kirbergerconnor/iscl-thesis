...
To evaluate all five models, precision, recall, and F1-score was calculated, given by Equations \ref{eq:precision}, \ref{eq:recall}, and \ref{eq:f1score}, respectively. These are standard metrics which are commonly used in machine learning tasks, including for supervised classification and unsupervised anomaly detection, making them a reasonable choice.
...
Due to training and predicting on the entire dataset, though, evaluating the unsupervised anomaly detection models was done just by calculating the metrics for each estimator during fine-tuning, without needing to keep count of scores. 
...
Figure \ref{fig:decisiontree} shows the resulting Decision Tree. To briefly explain how this is interpreted, for each node of the tree except the terminal ones, the top line is the condition for the subsequent split, and each split is a binary partition of the remaining number of samples. For example, the condition for the root node is \texttt{MTLD <= 70.824}, which is true for 9 samples and false for the other 15. In addition, \texttt{value} shows the weighted counts for both classes, with \texttt{class} being equal to the majority class at that split, and \texttt{gini} ranges from 0.0 (the samples all belong to one class) to 0.5 (the samples are evenly distributed, referred to as maximum impurity). 
...
\begin{figure}[!h]
  \centering
    \includegraphics[width=0.5\textwidth]{OCSVM_SHAP_values.png}
    \caption{SHAP Values of One-Class SVM}
    \label{fig:oneclasssvmshapvalues}
\end{figure}
