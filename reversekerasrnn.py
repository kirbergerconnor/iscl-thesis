# Used for calculating the reverse lexical entropy
...
reverse_lex_entropy_hist_df = pd.DataFrame(columns=['corpus', 'epoch', 'loss', 'acc', 'val_loss', 'val_acc'])
count = 0
...
if __name__ == '__main__':
    for filename in os.listdir(folder):
        if filename.endswith('.txt'):
            ...
            reverse_lex_entropy_hist_df = pd.concat([reverse_lex_entropy_hist_df, temp], ignore_index=True)
            
    with open('reverse_lex_entropy_hist_df.csv', mode='w') as f:
        reverse_lex_entropy_hist_df.to_csv(f)
